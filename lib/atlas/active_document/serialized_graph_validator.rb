  ...
  module ActiveDocument
    class SerializedGraphValidator < ActiveModel::Validator
      def validate(record)
        ...
        validate_slots(record)
      end

      private

      def skip_validations?(record)
        record.new_record? || record.errors.full_messages_for(:graph).any?
      end

      # Private: validate_nodes(record)
      #
      ...
      # Private: validate_edges(record)
      #
      # Adds an error for all missing edges.
      def validate_edges(record)
        ...
        @graph.graph.nodes.flat_map do |node|
          if slot_hash = @graph.graph_hash.fetch(:nodes)[node.key]
            ...
            if missing_slots.any?
              record.errors.add(:graph, "the following slots for #{ node.key } "\
                "are missing in the snapshot of the graph: "\
                "#{ missing_slots.join(", ") }")
            end
          end
        end
