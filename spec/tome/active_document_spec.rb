...
describe SomeDocument, :fixtures do
  ...
  describe "find" do
    ...
    it "should find by Symbol" do
      some_document = Tome::SomeDocument.find(:foo)
      expect(some_document.key).to eql(:foo)
    end
    ...
    it "loads a document from a subfolder" do
      another_document = Tome::SomeDocument.find(:bar)
    ...
    context 'when the key changed' do
      ...
      it 'no longer finds the old document' do
        ...
        expect { SomeDocument.find(:foo) }.
          to raise_error(Tome::DocumentNotFoundError)
      end
  ...
  describe '#update_attributes!' do
    ...
    context 'when validation fails' do
      ...
      it 'raises an error' do
        expect { result }.to raise_error(Tome::InvalidDocumentError, /Query/)
      end
    end
