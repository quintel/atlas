...
module Atlas
  describe SomeDocument do
    ...
    describe 'new' do
      ...
      context 'given a dumb key' do
        ...
        it 'sets the key (as a symbol)' do
          expect(SomeDocument.new(key: 'key').key).to be(:key)
        end
      ...
      context 'given a path' do
        it 'creates a new document' do
          ...
          expect(some_document.key).to be(:new)
        end

        it 'sets the key (as a symbol)' do
          expect(SomeDocument.new(path: 'a/b/thing').key).to be(:thing)
        end
        ...
        it 'saves in that folder' do
          some_document = SomeDocument.new(path: 'my_map1/new')
          expect(some_document.key).to be(:new)
          expect(some_document.path.to_s).to match %r{my_map1/new}
        end
    ...
    describe 'find' do
      it 'loads a some_document from file' do
        expect(some_document.key).to be(:foo)
    ...
    describe 'path' do
      it 'changes when the key has changed' do
        some_document.key = :total_co2_emitted
        expect(some_document.key).to be(:total_co2_emitted)
    ...
    describe '#update_attributes!' do
      ...
      context 'when validation fails' do
        ...
        it 'updates given attributes' do
          ...
          expect(&result_without_exception)
            .to change { document.attributes.values_at(:comments, :query) }
            .to(['Archer', nil])
        end
    ...
    describe '#<=>' do
      ...
      it 'is -1 when the node has an "earlier" key' do
        expect(Node.new(key: 'a') <=> node).to be(-1)
      end

      it 'is 0 when the node has an equal key' do
        expect(Node.new(key: 'f') <=> node).to be(0)
      end

      it 'is 1 when the node has a "later" key' do
        expect(Node.new(key: 'z') <=> node).to be(1)
      end
    end
